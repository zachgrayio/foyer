    ...
    P: Properties,
{
    ...
    async fn inspect_live_address(&self, hash: u64, address: EntryAddress) -> Result<Option<InspectedEntry<K>>> {
        ...
        if key_end != address.len as usize {
            return Err(Error::new(
                ...
                "indexed entry length does not match its header",
            ));
        }
    ...
    fn inspect_entry(&self, hash: u64) -> impl Future<Output = Result<Option<InspectedEntry<K>>>> + Send + 'static {
        ...
        async move {
            ...
            this.inspect_live_address(hash, address).await
        }
    }
    ...
    fn inspect_entries_page(
        ...
        async move {
            ...
            for entry in addresses {
                if let Some(entry) = this.inspect_live_address(entry.hash, entry.address).await? {
                    entries.push(entry);
                }
            }
    ...
    fn delete(&self, hash: u64) {
        ...
        let stats = removed.as_ref().map(|addr| InvalidStats {
            block: addr.block,
    ...
    P: Properties,
{
    ...
    fn inspect_entry_at(
        ...
        async move {
            ...
            if EntryAddressSnapshot::from(current.clone()) != address {
                return Ok(None);
            }
            this.inspect_live_address(hash, current).await
        }
        .boxed()
    }
...
mod tests {
    ...
    use std::{
        ...
        fs::File,
        path::Path,
        sync::{Mutex, atomic::AtomicBool as TestAtomicBool},
    };
    ...
    async fn test_storage_event_order_and_drop_count() {
        ...
        enqueue(&store, entry);
        store.wait().await;
        assert!(store.inner.observer.dropped() > 0);
    }
    ...
    async fn test_inspect_live_entries() {
        ...
        assert_eq!(
            Engine::inspect_entry_at(store.as_ref(), hash, inspected.address.clone())
                .await
                .unwrap(),
        ...
        assert!(
            Engine::inspect_entry_at(store.as_ref(), hash, stale_address)
                .await
                .unwrap()
                .is_none()
        );
        ...
        assert_eq!(store.inspect_block(block).await.unwrap().unwrap(), vec![inspected]);
    ...
    async fn test_force_reclaim_validates_generation_and_removes_entries() {
        ...
        let snapshot = snapshots
            .iter()
            .find(|snapshot| snapshot.state == BlockState::Evictable && snapshot.live_entries == 2)
            .cloned()
        ...
        let reclaimed = store
            ...
            .unwrap();
        assert_eq!(reclaimed.state, BlockState::Clean);
